...
- `src/` is the page. `app.rs` creates the engine handle with `use_engine` and composes `EngineViewport`, the HUD, and the loader from `nightshade-leptos`; `state.rs` is the game-specific page state as `Copy` signals; renderer facts (ready, adapter, FPS, entities, selection) arrive on the handle's reactive `EngineState`.
