...
check:
    cargo check -p protocol -p worker -p template_api_leptos --target wasm32-unknown-unknown
    cargo check -p desktop
    cargo fmt --all -- --check

# Runs clippy across the workspace and denies warnings
lint:
    cargo clippy -p protocol -p worker -p template_api_leptos --target wasm32-unknown-unknown -- -D warnings
